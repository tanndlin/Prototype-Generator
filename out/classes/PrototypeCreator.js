...
class PrototypeCreator {
    ...
    static execute() {
        ...
        // Get prototypes
        const existingPrototypes = PrototypeCreator._getFunctionPrototypes(document, mainLine); // All prototypes are before main function
    ...
    static _getFunctionPrototypes(document, mainLine) {
        const prototypes = [];
        for (let i = 0; i < mainLine; i++) {
            ...
            const prototypeRegex = /[_\w\d]+ [_\w\d]+\(([_\w\d]+ \*?[_\w\d]+(, [_\w\d]+ \*?[_\w\d]+)*)?\);/g;
